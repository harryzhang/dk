        ...
        for (var idx = 0; idx<st.length; idx++) {
          if (st[idx] != '' && st[idx] != 'undefined') {
            if(st[idx].agreementFileName) {
              ...
              htm += '<tr><td colspan="2" style="text-align:left;padding-left:22px;border:0;color: #CCC;">订单号:'+st[idx].trade_id+'</td><td colspan="3" style="text-align:right;padding-right:22px;border:0;color: #CCC;">购买时间:'+st[idx].trade_time_info+'</td></tr>'+'<tr><td>+車宝' + st[idx].period + '月</td>' +
              '<td>￥' + st[idx].trade_amount + '</td>';
            }
            ...
            if('SUCCESS'==status){
              if(st[idx].canRedeem==1){
                ...
                '<td>投资中</td>'+
                '<td><a href="<c:url value="/redeemPagePhone.do"/>?tradeId='+st[idx].trade_id+'" target="_self" style="color:blue;">提前赎回</a></td></tr>';
