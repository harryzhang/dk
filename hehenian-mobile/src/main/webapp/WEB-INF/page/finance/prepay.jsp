...
<body class="bg-2">
	<header class="top-bar bg-f">
		<a href="http://m.hehenian.com/product/plist.do"> <span
			class="icon-back2"> </span>
		</a>
		<p>购买</p>
	</header>
	<div class="prodetail-top">
		<span class="rate-num">${product.rateInfo * 100 }%</span>
		<p class="prohead-name">${product.product_name} ·
			${product.period}-M</p>
	...
	<section class="sign-area margin-none">
		<form>
			<div class="sign-style br-3 bb-1">
				<span class="sign-lable">购买金额：</span> <input type="text" id="amount"
					maxlength="7" placeholder="${product.coefficient }元起购"
					oninput="getExpectedReturn()"
					onpropertychange="getExpectedReturn()">
			</div>
			<div class="buy-tips">
				<p>
					预期收益：<span id="expectedReturn">0</span>元
				</p>
				<p>
					计息日期：<span id="interestDate"></span>
				</p>
			</div>
			<div class="sign-style br-2 bb-1">
				<span class="sign-lable">选择银行：</span>
				<c:if test="${not empty bingCard.bank_card_id }">
					<a
						href="http://m.hehenian.com/profile/managerCard.do?pid=${product.product_rate_id}"
						class="cb-a" val="${bingCard.bank_card_id}">${bingCard.bank_name}(尾号${bingCard.card_no})</a>
				</c:if>
				<c:if test="${ empty bingCard.bank_card_id }">
					<a href="http://m.hehenian.com/finance/bindCard.do" class="cb-a" style="text-decoration: underline;">添加银行卡</a>
				</c:if>
			</div>
			<div class="buy-tips">
				<p class="tips-red">
					账户余额：<span id="balance">${balanceAmount}</span>元<span
						class="icon-atten"></span>
				</p>
			</div>
			<c:if
				test="${ !(product.channel eq 1 and product.sub_channel eq 1) }">
				<div class="sign-style br-1 bb-1">
					<span class="sign-lable">推荐人：</span> <input type="tel"
						placeholder="(选填)请输入推荐人手机号" id="recPhone" name="recPhone"
						maxlength="11">
				</div>
			</c:if>
			<p>
				<span></span>同意<a href="http://m.hehenian.com/finance/agreement.do">《${product.product_name}购买协议》</a>
			</p>
			<div class="bnb-a">
				<input type="hidden" id="totalAm" value="${canInvest}" /> <input
					type="button" value="立即购买" class="bn-btn" id="buyBtn">
			</div>
		</form>
	</section>
...
// 				$('.buy-pop').show();
				...
				$('.buy-succ p:eq(0)').text(
						'您成功购买爱定宝·' + mouth + '-M，购买金额' + invest + '元。');
...
// 				$('.buy-succ .bs-foot').hide();
		...
		$(function() {
			...
			$("#succ_close").bind("click", function() {
				window.location.href="http://m.hehenian.com/profile/buy.do?flag=xin&tab_f=g";
			});
