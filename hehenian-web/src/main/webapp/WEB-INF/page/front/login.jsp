...
<div style=" margin-top:30px; margin-bottom:90px;">
  <div style=" width:900px; border:1px solid #e8e8e8; background:#FFF; margin:0px auto; overflow:hidden; height:400px;">
...
if (ad==1){
alt="ad1";
banner="<c:url value='/newimages/login1.jpg'/>";
width="540";
height="400";
}
if (ad==2) {
alt="ad2";
banner="<c:url value='/newimages/login2.jpg'/>";
width="540";
height="400";
}
if (ad==3) {
alt="ad3";
banner="<c:url value='/newimages/login3.jpg'/>";
width="540";
height="400";
}
...
--></script>
    ...
    <div style=" width:290px; float:left; padding:30px;">
      ...
      <ul>
        ...
        <li>
          ...
          <img src="<c:url value='/admin/imageCode.do?pageId=userlogin'/>" title="点击更换验证码"
        ...
        <li  style=" margin-top:10px; line-height:38px;">
          ...
          记住我的帐号</b>&nbsp;&nbsp;<a style="color:#f60;" href="<c:url value='/forgetpassword.do'/>">忘记密码</a></li>
        <li>
  ...
  <div style=" width:870px; margin:0px auto; overflow:hidden;padding-right:30px;">
    <div style=" background:#7cbe56; border:5px solid #e8e8e8; border-top:0px; width:150px; text-align:center; font-size:12px; color:#fff; line-height:32px; float:right; "> <a href="<c:url value='/account/reg.do'/>${empty param.fromUrl ? '':'?fromUrl='}${param.fromUrl}${empty param.source ? '':'&source='}${param.source}" style=" color:#FFF">免费注册>> </a></div>
  </div>
</div>
...
<script>
		function switchCode() {
			...
			$("#codeNum").attr("src", "<c:url value='/admin/imageCode.do?pageId=userlogin&d='/>" + timenow);
		}
...
<script>
		function login() {
			...
			if ($("#email").val() == "") {
				$("#s_email").attr("class", "formtips onError");
				$("#s_email").html("<img src='<c:url value='/images/v1/login.png'/>'>请输用户名或邮箱地址");
			}
			if ($("#password").val() == "") {
				$("#s_password").attr("class", "formtips onError");
				$("#s_password").html("<img src='<c:url value='/images/v1/login.png'/>'>请输入密码");
			...
			$.post("<c:url value='/account/login.do'/>" , param, function(datas) {
				var data=datas.msg;
				if (data == 1) {
					var fromUrl = "${param.fromUrl}";
                    if(fromUrl==""){
                        fromUrl = datas.fromUrl;
                    }else{
...
//					var fromUrl = datas.fromUrl;
					...
					$("#s_code").attr("class", "formtips onError");
					$("#s_code").html("<img src='<c:url value='/images/v1/login.png'/>'>验证码错误！");
					switchCode();
					$("#btn_login").attr('disabled', false);
				} else if (data == 3) {
					$('#btn_login').attr('value', '登录');
					$("#s_email").attr("class", "formtips onError");
					$("#s_email").html("<img src='<c:url value='/images/v1/login.png'/>'>用户名或密码错误！");
					switchCode();
					$("#btn_login").attr('disabled', false);
				} else if (data == 4) {
					$('#btn_login').attr('value', '登录');
					$("#s_email").attr("class", "formtips onError");
					switchCode();
					$("#s_email").html("<img src='<c:url value='/images/v1/login.png'/>'>该用户已被禁用！");
					$("#btn_login").attr('disabled', false);
				} else if (data == 200) {
					$("#s_email").attr("class", "formtips onError");
					switchCode();
		...
		function reSendEmail() {
			...
			window.location.href = "<c:url value='reActivateEmail.do?email='/>" + $("#email").val();
		}
