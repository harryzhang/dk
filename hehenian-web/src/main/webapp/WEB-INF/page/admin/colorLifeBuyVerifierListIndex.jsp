...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#excel").click(function(){
			...
			$('#beginDateHid').val($('#beginDate').val());
			$('#endDateHid').val($('#endDate').val());
			$('#productPeriodHid').val($('#productPeriod').val());
			$('#exportEexelForm').submit();
		});
	});
	...
	function initListInfo(praData) {
		...
		param["paramMap.endDate"] = $("#endDate").val();
		param["paramMap.productPeriod"] = $("#productPeriod").val();
		$.post("admin/listColorLifeVerifierInfo.do", praData, initCallBack);
	}
	...
	function changePage(pageSelect) {
		...
		param["paramMap.endDate"] = $("#endDate").val();
		param["paramMap.productPeriod"] = $("#productPeriod").val();
		$.post("admin/listColorLifeVerifierInfo.do", param, initCallBack);
	}
...
<form action='<c:url value="exportExcelBuyInfo.do"/>' method="post" id="exportEexelForm">
	<input type="hidden" name="pageBean.pageNum" id="pageNum">
	<input type="hidden" name="pageBean.pageSize" id="pageSize">
	<input type="hidden" name="paramMap.beginDate" id="beginDateHid">
	<input type="hidden" name="paramMap.endDate" id="endDateHid">
	<input type="hidden" name="paramMap.productPeriod" id="productPeriodHid">
</form>
		...
		<div style="padding: 15px 10px 0px 10px;">
			...
			<div
				...
				<form id="form" name="form"
						action="<c:url value="/admin/listColorLifeVerifierInfo.do"/>" method="post">
							...
							<tbody>
								<tr>
									<td class=f66 height=36 width="50%" align=left>
										...
										&nbsp;&nbsp;起止时间： <input name="paramMap.beginDate" id="beginDate"
