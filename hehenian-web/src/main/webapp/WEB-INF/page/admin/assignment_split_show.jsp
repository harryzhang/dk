...
<html>
	...
	function count()
	{
		...
		if($.trim($("#number").val()).length != 0)
			{
				$.post("queryDebtSumById.do?id="+id+"&number="+number, function(data) {
	...
	function queryRealAmount(){
		...
		$.post("queryRealAmount.do?id="+id+"&realAmount="+realAmount, function(data) {
			if(data=="2"){
				$("#realAmount").attr("value","");
		...
		<table
			...
			<tr>
				<td style="width: 150px;" align="right">
					拆分方式：<span class="require-field">*&nbsp;</span>
				</td>
				<td style="width: 200px;">
			...
			<tr>
				<td align="right">
					拆分份数：<span class="require-field">*&nbsp;</span>
				</td>
				<td>
					<input id="number" name="paramMap.number" onblur="count();" type="text" onkeyup="value=value.replace(/[^\d]/g,'')" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))" ></input>
					<span class="#000000">*&nbsp;轻输入2-49的正整数</span>
				</td>
			</tr>
			<tr>
				<td align="right">
					拆分金额：<span class="require-field">*&nbsp;</span>
				</td>
				<td>
					<input id="realAmount" name="paramMap.realAmount" type="text" onblur="queryRealAmount();"
			...
			<tr>
				<td align="center" colspan="2">
					<input type="button" style="background: #666666;" onclick="assignmentSplit();" value="确定" />
