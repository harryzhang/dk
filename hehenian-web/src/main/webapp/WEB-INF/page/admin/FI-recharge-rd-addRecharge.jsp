...
<script type="text/javascript">
	$(function() {
		...
		$("input").blur(function() {
			if ($(this).attr("id") == "tb_money") {
				if ($(this).val() == "") {
					$(this).next().html("奖励金额不能为空");
					return;
				} else {
					$(this).next().html("");
				}
				if (isNaN($(this).val())) {
					$(this).next().html("奖励金额为数字");
					return;
				} else {
					$(this).next().html("");
				}
				if ($(this).val() <= 0) {
					$(this).next().html("奖励金额为正数");
					return;
				} else {
					$(this).next().html("");
				}
	...
	function tb_submit() {
		...
		$.shovePost("addBackRecharge.do", param, function(data) {
			alert(data);
			if (data == "奖励成功") {
	...
	<div style="padding: 15px 0px 0px 0px;width: 92%;margin: 0 auto;">
		<div style="padding-right: 10px; padding-left: 10px; padding-bottom: 10px; width: 100%; padding-top: 10px; background-color: #fff;">
			<table width="92%" border="0" cellspacing="1" cellpadding="3">
				...
				<tr>
					<td style="width: 120px; height: 35px;" align="right" class="blue12">奖励金额：</td>
