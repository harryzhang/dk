...
import com.sp2p.database.Dao.Tables.t_role;
...
public class UserManageDao {
   ...
   */
	 ...
	 * @throws DataException
	 */
	public Long updateUserAccountType(Connection conn,Long userId) throws SQLException, DataException{
		Dao.Tables.t_user user = new Dao().new Tables().new t_user();
		Map<String,String> userMap = null;
		DataSet dataSet = user.open(conn, "accountType", " id = "+userId, "", -1, -1);
		userMap = BeanMapUtils.dataSetToMap(dataSet);
		Integer accountType = null;
		if(userMap!=null&&userMap.size()>0){
			accountType = Convert.strToInt(userMap.get("accountType"), -1) ;
			if(accountType == 1){
				user.accountType.setValue(0);
			     return  user.update(conn, " id = "+userId);
			}else{
				user.accountType.setValue(1);
			     return  user.update(conn, " id = "+userId);
			}
		}
		return -1L;
	 ...
	 * @throws DataException
	 */
	public Map<String,String> queryUserAccountType(Connection conn,Long userId) throws SQLException, DataException{
		String sqlStr = "SELECT accountType from t_user where id="+userId;
