	...
	public String alipayReceive() throws Exception {
		...
		try {
			...
			Map requestParams = (Map<String, String>) JSONObject.toBean(
					JSONObject.fromObject(appInfoMap.get("params")), HashMap.class);
...
//				createHelpMessage("支付失败！", "返回首页", "index.do");
			...
			String paynumber = URLDecoder.decode(appInfoMap.get("trade_no"), "utf-8");
			// 支付宝编号
			String notify_time = URLDecoder.decode(appInfoMap.get("notify_time"), "utf-8");// 支付宝编号
			...
			String buyer_email = URLDecoder.decode(appInfoMap.get("buyer_email"), "utf-8");// 支付银行
			String buyer_id = URLDecoder.decode(appInfoMap.get("buyer_id"), "utf-8");// 支付银行
