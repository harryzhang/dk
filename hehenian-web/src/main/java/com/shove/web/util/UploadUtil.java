...
public class UploadUtil {
	...
	public static String getByAllParams(Files file, FileCommon fileCommon,
		...
		/*
		 * //若指定了类型，则进行检查 if (StringUtils.isNotBlank(fileCommon.getFileType()))
		 * { if
		 * (!fileCommon.getFileType().toUpperCase().contains(getExt.toUpperCase
		 * ())) { return "文件类型不对!"; } }
		 * 
		 * //若禁止类型，则进行检查 String notAllowFileType =
		 * fileCommon.getNotAllowFileType(); if
		 * (StringUtils.isNotBlank(notAllowFileType)) { if
		 * (notAllowFileType.toUpperCase().contains(getExt.toUpperCase())) {
