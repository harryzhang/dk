...
<html>
	...
	<body>
		<section>
			<header>
				...
				<div class=" db_f manage_nav">
					...
					<a href="<c:url value='/app/mhk/loanManage/stillRepayPage.do'/>" class="bf1 t_ac check"><em>待还款</em></a> 
					<a href="<c:url value='/app/mhk/loanManage/hasRepayPage.do'/>" class="bf1 t_ac"><em>已还款</em></a>
					<a href="<c:url value='/app/mhk/loanManage/repayedPage.do'/>" class="bf1 t_ac"><em>已还清</em></a>
				</div>
			</header>
			<article>
				<section id="dataBody">
		...
		function loadData(){
			...
			$.get('<c:url value="/app/mhk/loanManage/stillRepay.do"/>',{searchStr:searchStr,pageSize:30,pageNum:1}, function (result) {
				if(result.status == 1){
				...
				var str = '';
	       		for(var i in data) {
	       				...
	       				str+='<a href="'+'<c:url value="/app/mhk/loanManage/repayDetail.do?detailType=2&loanId='+data[i].loanId+'&repayPeriod='+data[i].repayPeriod+'"/>'+'" class="btn">查看</a>';
	       				str+='</div></article>';
	       		}
				$("#dataBody").html(str);
			});
